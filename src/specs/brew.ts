...
const servicesGenerator = (action: string): Figy.Generator => ({
...
const repositoriesGenerator = (): Figy.Generator => ({
  script: ["brew", "tap"],
  postProcess: (out) => {
    return out.split("\n").map((line) => ({ name: line }));
  },
...
const formulaeGenerator: Figy.Generator = {
  script: ["brew", "list", "-1"],
  postProcess: function (out) {
    return out
      .split("\n")
...
const outdatedformulaeGenerator: Figy.Generator = {
...
const generateAllFormulae: Figy.Generator = {
...
const generateAllCasks: Figy.Generator = {
  script: ["brew", "casks"],
  postProcess: function (out) {
    return out.split("\n").map((cask) => ({
      name: cask,
...
const generateAliases: Figy.Generator = {
  script: [
    "bash",
    "-c",
...
const commonOptions: Figy.Option[] = [
  {
    name: ["-d", "--debug"],
